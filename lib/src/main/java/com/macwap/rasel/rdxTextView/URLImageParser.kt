...
class URLImageParser(var container: View, var context: Context, var textView: RDxTextView) : Html.ImageGetter {
	...
	inner class ImageGetterAsyncTask(private var urlDrawable: URLDrawable) : AsyncTask<String?, Void?, Drawable?>() {
		...
		override fun onPostExecute(result: Drawable?) {
			result?.intrinsicWidth?.let { urlDrawable.setBounds(0, 0, it, result.intrinsicHeight)  //set the correct bound according to the result from HTTP call
